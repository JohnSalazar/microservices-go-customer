...
import (
	...
	customer_nats "customer/src/nats"
	"customer/src/repositories"
	"customer/src/routers"
...
func startup(ctx context.Context) (*Main, error) {
	...
	subjects := []string{string(common_nats.UserDeleted)}
	js, err := common_nats.NewJetStream(nc, "authentication", subjects)
	if err != nil {
		log.Fatalf("Nats JetStream create error: %+v", err)
	}

	natsPublisher := common_nats.NewPublisher(js)
	natsMetrics := customer_nats.NewNatsMetric(config)
	...
	listens := customer_nats.NewListen(js, customerService, emailService, natsPublisher, natsMetrics)
	listens.Listen()
	...
	customerController := controllers.NewCustomerController(
		...
		customerService,
		addressService)
	router := routers.NewRouter(config, metricService, authentication, customerController)
